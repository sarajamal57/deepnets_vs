   ...
   ---------------------------------------------------------------------------"""   
    ...
    if type == 'deconvolution' :
        x = Conv1DTranspose(x,
                            ...
                            padding = 'same',
                            name = name + 'B{}_L{}_dilC{}_N{}'.format(m_stack, m_dilation_rate, kernel_size, sizenet))
    else:
        x = Conv1D(filters = sizenet, 
                   kernel_size = kernel_size, 
                   dilation_rate = m_dilation_rate,
                   padding = m_padding,
                   name = name + 'B{}_L{}_dilC{}_N{}'.format(m_stack, m_dilation_rate, kernel_size, sizenet))(x)
...
## --------------------------------------------------------------- ##
## DECONVOLUTION 1D (:= transpose convolution), Flatten 2D representation
## --------------------------------------------------------------- ##
        ...
        for i in range(num_layers):
            ...
            if bidirectional:
                x = wrapper(layer(units = sizenet, 
                        ...
                        name = 'encode_pb{}_x{}'.format(j,i)) (x)
            else:
                x = wrapper(layer(units = sizenet, 
                              return_sequences = True if return_sequences else (i<num_layers-1),
                              name = 'encode_pb{}_x{}'.format(j,i))) (x)
            if drop_frac > 0.0:
                x = Dropout(rate = drop_frac,  
                            name = 'encode_drop_pb{}_x{}'.format(j,i))(x)
        
        ## Output per band ##
        x_list.append(x)
    
    #---------------CLASSIFIER---------------#
    x_merged = concatenate(x_list, name = 'encode_concat', axis = 1) if nb_passbands>1 else x_list[0]
    if not return_sequences:
        x_merged = concatenate([x_merged, meta_input])  if (add_meta is not None) else x_merged
        x_merged = Dense(units = relu_size, 
                         activation = 'relu', 
                         name = 'relu_dense')(x_merged) 
        x_merged = Dense(units = output_size, 
                         activation = 'softmax',
                         name = 'clf_softmax_dense')(x_merged) 
    else:
        if (nb_passbands>1):
            x_merged = Dense(units = relu_size, 
                             activation = 'relu',  
                             name = 'relu_dense')(x_merged) 
        x_merged = TimeDistributed(Dense(units = output_size, 
                                         activation = 'linear'), 
                                   name = 'time_dist'.format(j))(x_merged)
...
def classifier_direct_tCNN(input_dimension, output_size, sizenet,
    ...
    for j in range(nb_passbands):
        ...
        for i in range(num_layers):
            ...
            ## Convolution ##
            x = Conv1D(filters =size_filter, ##### sizenet, #//(2**(i)),  
                       kernel_size = kernel_size, 
                       strides = m_reductionfactor,
                       activation = m_activation, #'relu'
                       input_shape = (max_lenght, input_dimension),
                       padding = m_padding,
                       name = 'encode_pb{}_x{}_conv1d'.format(j, i))(x)  
            
            ## Dropout ##
            if drop_frac>0:
                x = SpatialDropout1D(drop_frac, 
                                     name = 'encode_pb{}_x{}_sdrop{}'.format(j, i, int(drop_frac*100)))(x)
                
        ## Output per band ##
        if (not return_sequences): 
            if max_lenght is None:   ### 050620
                 x = Lambda(lambda y: y[:, -1, :], name='encode_pb{}_lambda'.format(j))(x)  ### 050620
            else:
                 x = Flatten(name = 'encode_pb{}_flat'.format(j))(x) 
    ...
    if not return_sequences:
        x_merged = concatenate([x_merged, meta_input]) if (add_meta is not None) else x_merged
        x_merged = Dense(units = relu_size, activation = 'relu', name = 'relu_dense')(x_merged)
        x_merged = Dense(units = output_size, activation = 'softmax', name = 'clf_softmax_dense')(x_merged)
    else:
        if (nb_passbands>1):
             x_merged = Dense(units = relu_size, activation = 'relu', name = 'relu_dense')(x_merged) 
    
        x_merged = TimeDistributed(Dense(units = output_size, activation = 'linear'), name = 'time_dist'.format(j))(x_merged)
...
def classifier_direct_dTCN(input_dimension, output_size, sizenet,
    ...
    for j in range(nb_passbands):
        ...
        x = Conv1D(filters = sizenet,
                   kernel_size = kernel_size,
                   padding = m_padding,
                   name = 'encode_pb{}_conv1d'.format(j))(x)
        x = SpatialDropout1D(rate = drop_frac, 
                             name = 'encode_pb{}_sdrop{}'.format(j, int(drop_frac*100)))(x)
        
        ## Residuals ##
        skip_connections = []
        for m_stack in range(n_stacks):
            for m_dilation_rate in m_dilations:
                x, skip_x = residual_block(x, sizenet, m_dilation_rate, m_stack, causal,
                                           m_activation, drop_frac, kernel_size, name = 'encode_pb{}_'.format(j))
                skip_connections.append(skip_x)
        if use_skip_connections:
            x = keras.layers.add(skip_connections, name = 'encode_pb{}_add_skip_connections'.format(j))
            
        ## Wavenet config ##
        x = Activation('relu', name = 'encode_pb{}_relu'.format(j))(x)
        if config_wavenet:
            x = Conv1D(filters = sizenet, 
                       kernel_size = kernel_wavenet, 
                       padding = 'same',
                       activation = 'relu',
                       name = 'encode_pb{}_n{}_cv{}_cw'.format(j, sizenet, kernel_wavenet))(x)
            
            x = Conv1D(filters = output_size_cw,
                       kernel_size = kernel_wavenet, 
                       padding = 'same',
                       name = 'encode_pb{}_n{}_cvw{}_cw'.format(j, output_size_cw, kernel_wavenet))(x)    
